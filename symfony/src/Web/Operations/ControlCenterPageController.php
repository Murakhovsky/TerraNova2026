...
use App\Application\Operations\Query\GetControlCenterQuery;
...
final readonly class ControlCenterPageController
{
    ...
    public function index(Request $request): Response
    {
        ...
        try {
            $data = $this->queries->ask(new GetControlCenterQuery(
                $tenant->organizationId(),
                max(1, min(100, (int) $request->query->get('limit', 30))),
            ));
            $control = $this->presenter->present(is_array($data) ? $data : [], $actionStatus);
    ...
    {
        return new Response(
            $this->twig->render('experience/system/control_center.html.twig', $variables),
            $status,
            [
