...
final readonly class ArchitecturePageController
{
    ...
    public function index(Request $request): Response
    {
        ...
        try {
            ...
            $graph = [
                ...
                'default_view' => $this->projections->has('system') ? 'system' : ($names[0] ?? ''),
            ];
        ...
        return new Response(
            $this->renderer->render($request, 'visualization/architecture', [
                'title' => 'COS Architecture Explorer',
                'metaTitle' => 'COS Architecture Explorer',
                'workspaceSection' => 'cos',
                'workspaceActive' => 'architecture',
                'workspaceActiveSection' => $this->navigation->activeSection('architecture'),
    ...
    public function graph(Request $request): Response
    {
        ...
        try {
            ...
            $stage = 'project_' . $name;
            $description = $this->projections->descriptions()[$name] ?? [
                'label' => $name,
                'layout' => 'auto',
                'default_depth' => null,
            ];
            ...
            $diagnostic = $this->failureDiagnostic($stage, $error);
            error_log(sprintf(
                '[COS Visualization] Architecture Explorer graph failed at %s: %s: %s',
                $stage,
                $error::class,
                $error->getMessage(),
            ));
    ...
    public function health(): Response
    {
        ...
        try {
            ...
            return new JsonResponse([
                'ok' => true,
                'health' => $this->health->analyze($graph),
            ]);
        } catch (Throwable $error) {
            $diagnostic = $this->failureDiagnostic($stage, $error);
            error_log(sprintf(
                '[COS Visualization] Architecture Explorer health failed at %s: %s: %s',
                $stage,
                $error::class,
                $error->getMessage(),
            ));
