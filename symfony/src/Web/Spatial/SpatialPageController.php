...
final readonly class SpatialPageController
{
    ...
    public function manage(Request $request):Response
    {
        ...
        return $this->workspaceResponse($request,$tenant,'Spatial / 3D','spatial/manage',[
            'filters'=>$filters,
            'scenes'=>$this->scenes->managerScenes($filters),
            'stats'=>$this->scenes->stats(),
            'pageStatus'=>(string)$request->query->get('status_message',''),
        ]);
    }
    ...
    public function upload(Request $request,string $id):Response
    {
        ...
        try {
            $asset=$this->scenes->upload(
                $sceneId,
                $this->uploadPayload($request->files->get('asset_file')),
                $request->request->all(),
                $this->actor($tenant),
            );
            ...
            return $this->statusRedirect('/spatial/edit/'.$sceneId,'Asset завантажено та поставлено в обробку.');
        } catch(Throwable $error) {
            if($this->wantsJson($request)) {
                return new JsonResponse(['ok'=>false,'message'=>$error->getMessage()],422);
            }
            return $this->statusRedirect('/spatial/edit/'.$sceneId,$error->getMessage());
        }
    }
    ...
    public function external(Request $request,string $id):Response
    {
        ...
        return $this->statusRedirect('/spatial/edit/'.(int)$id,$message);
    }
    ...
    public function publish(string $id):Response
    {
        ...
        return $this->statusRedirect('/spatial/edit/'.(int)$id,(string)($result['message'] ?? 'Spatial scene processed.'));
    }

    public function scene(Request $request,string $slug):Response
    {
        $scene=$this->scenes->publicScene($slug);
        if($scene===null) return new Response('Spatial scene not found.',404);
        ...
        $variables=[
            ...
            'metaImage'=>$scene['viewer']['poster_url'] ?? null,
        ...
        return new Response($this->renderer->render($request,'spatial/scene',$variables),200,['Content-Type'=>'text/html; charset=UTF-8']);
    }
    ...
    private function workspaceResponse(Request $request,TenantContext $tenant,string $title,string $view,array $extra=[]):Response
    {
        $role=$tenant->role()->value();
        $variables=array_replace([
            'title'=>$title,
            'metaTitle'=>$title.' | Terra Nova CLUB',
            'metaRobots'=>'noindex,nofollow',
            'workspaceSection'=>'property',
            'workspaceActive'=>'spatial',
            'workspaceActiveSection'=>$this->navigation->activeSection('spatial'),
            'pageAssetEntries'=>[],
        ...
        return new Response($this->renderer->render($request,$view,$variables),200,['Content-Type'=>'text/html; charset=UTF-8']);
    }
    ...
    private function uploadPayload(mixed $file):array
    {
    ...
    private function statusRedirect(string $target,string $message):RedirectResponse
    {
    ...
    {
        return str_contains(mb_strtolower((string)$request->headers->get('Accept','')),'application/json')
            || mb_strtolower((string)$request->headers->get('X-Requested-With',''))==='xmlhttprequest';
    }
}
