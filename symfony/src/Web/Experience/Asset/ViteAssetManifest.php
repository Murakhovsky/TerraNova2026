...
namespace App\Web\Experience\Asset;
...
final class ViteAssetManifest
{
    ...
    {
        $scripts=[]; $styles=[];
        foreach (array_values(array_unique(array_filter($entries))) as $entry) {
            $asset=$this->entry((string)$entry);
            $scripts[]=$this->url((string)$asset['file']);
            foreach ((array)($asset['css']??[]) as $css) $styles[]=$this->url((string)$css);
        }
    ...
    private function entry(string $name): array
    {
        ...
        $source='frontend/spatial/'.$name.'.js';
        if (isset($manifest[$source]) && is_array($manifest[$source])) return $manifest[$source];
        throw new RuntimeException(sprintf('Specialized Vite island "%s" is absent from %s.',$name,$this->manifestPath));
    }
    ...
    private function manifest(): array
    {
        ...
        if (!is_file($this->manifestPath)) throw new RuntimeException('Vite manifest is missing.');
        $decoded=json_decode((string)file_get_contents($this->manifestPath),true,512,JSON_THROW_ON_ERROR);
        if (!is_array($decoded)) throw new RuntimeException('Vite manifest must contain a JSON object.');
