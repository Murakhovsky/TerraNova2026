...
final readonly class AnswerDiagnosticInterviewCommandHandler implements CommandHandlerInterface
{
    ...
    {
        $org=$command->organizationId->value(); $result=$this->runtime->answer($org,$command->sessionId,$command->answer,(string)$command->actorId,$command->idempotencyKey);
        if(($result['replayed']??false)!==true)$this->audit->record($org,$command->actorId,$command->correlationId,'diagnostic.interview_answered',$command->sessionId);
        return $result;
    }
}
