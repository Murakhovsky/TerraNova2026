...
final readonly class AcceptDiagnosticRecommendationCommandHandler implements CommandHandlerInterface
{
    ...
    {
        $org=$command->organizationId->value(); $result=$this->runtime->accept($org,$command->sessionId,$command->recommendationId);
        if(($result['replayed']??false)!==true)$this->audit->record($org,$command->actorId,$command->correlationId,'diagnostic.recommendation_to_action',$command->sessionId,['recommendation_id'=>$command->recommendationId,'action_id'=>$result['action_id']??null]);
        return $result;
    }
}
