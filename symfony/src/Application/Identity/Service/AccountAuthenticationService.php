...
final readonly class AccountAuthenticationService
{
    ...
    /** @param array<string,mixed> $input @return array{ok:bool,message:string,user?:array<string,mixed>} */
    ...
    public function register(array $input): array
    {
        ...
        if ($user === null) {
            return ['ok' => false, 'message' => 'Акаунт створено, але його не вдалося відкрити.'];
        }
        $this->touchLogin($id);
    ...
    private function findByEmail(string $email): ?array
    {
        ...
        $statement->execute(['organization_id' => $this->organization(), 'email' => $email]);
    ...
    private function findById(int $id): ?array
    {
        ...
        $statement->execute(['organization_id' => $this->organization(), 'id' => $id]);
        $row = $statement->fetch(PDO::FETCH_ASSOC);
    ...
    {
        $organization = trim($this->organizationId);
        return $organization !== '' ? $organization : 'default';
    }
}
