...
final readonly class LegacyIdentityRepositoryAdapter implements IdentityResolverInterface
{
    ...
    {
        $legacyUserId = $userId->value();
        if (!ctype_digit($legacyUserId) || (int) $legacyUserId <= 0) {
            return null;
        }
        ...
        $statement->execute(['id' => (int) $legacyUserId]);
