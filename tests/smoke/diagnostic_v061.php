...
$assert(($manifest['contributions']['api_route_contributor_services'] ?? []) === [], 'Diagnostic domain manifest must not restore Phalcon route contributors.');
...
foreach ([
    ...
    $assert(!is_file($root . '/' . $retired), 'Retired Phalcon Diagnostic delivery restored: ' . $retired);
}
...
$assert(!str_contains($webServices, 'DiagnosticModuleRouteContributor'), 'Legacy Diagnostic route contributor leaked back into Web composition.');
$assert(!str_contains($webServices, "setShared('diagnosticRouteContributor'"), 'Legacy Diagnostic route contributor service leaked back into Web composition.');
...
$assert(str_contains($reportController, 'DiagnosticRuntimeService'), 'HTML report is not backed by canonical runtime report persistence.');
$assert(str_contains($reportController, 'DiagnosticMethodologyAccess'), 'Methodology Studio must enforce the canonical Diagnostic permission service.');
$assert(str_contains($reportController, "['diagnostics-methodology-studio']"), 'Methodology Studio must load its Vite feature entrypoint.');
$assert(!str_contains($reportController, 'Phalcon\\'), 'Symfony Diagnostic page controller must not depend on Phalcon.');
