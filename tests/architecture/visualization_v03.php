...
$assert(!str_contains($controller, 'Infrastructure\\'), 'Symfony Web controller must not compile against Infrastructure implementations.');
...
$assert(str_contains($controller, 'GraphMapperInterface'), 'Explorer must consume the renderer-neutral graph mapper contract.');
...
$assert(!str_contains($module, 'VisualizationRoutes::register($router)'), 'Retired Phalcon Visualization routes were restored.');
$symfonyServices = $read('symfony/config/services.yaml');
$assert(str_contains($symfonyServices, 'Kernel\\Visualization\\Graph\\GraphMapperInterface:'), 'Graph mapper port is not composed in Symfony.');
