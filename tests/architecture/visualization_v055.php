...
$assert(str_contains($services, 'Kernel\\Visualization\\Graph\\GraphHealthAnalyzerInterface:'), 'Symfony composition must register graph health contract.');
$assert(str_contains($services, 'Infrastructure\\Visualization\\Architecture\\ArchitectureGraphHealthAnalyzer'), 'Symfony composition must register graph health implementation.');

$assert(str_contains($controller, 'GraphHealthAnalyzerInterface'), 'Symfony controller must depend on the Kernel health contract.');
$assert(str_contains($controller, 'public function health('), 'Manager health JSON endpoint is missing.');
$assert(str_contains($controller, "\$this->health->analyze"), 'Controller must invoke the injected graph health analyzer.');
$assert(str_contains($controller, "'health' => \$this->health->analyze"), 'Health endpoint must return the health payload.');
$assert(!str_contains($controller, 'Phalcon\\'), 'Symfony Architecture controller must not depend on Phalcon.');

$assert(str_contains($routes, 'cos_web_architecture_health:'), 'Architecture health Symfony route is missing.');
$assert(str_contains($routes, 'ArchitectureExplorerController::health'), 'Architecture health route must target the Symfony health method.');
