...
$assert(str_contains($services, 'Kernel\\Visualization\\Graph\\GraphHealthAnalyzerInterface:'), 'Symfony composition must register graph health analyzer.');
$assert(str_contains($services, 'alias: Infrastructure\\Visualization\\Architecture\\ArchitectureGraphHealthAnalyzer'), 'Graph health analyzer alias missing.');

$assert(str_contains($controller, 'GraphHealthAnalyzerInterface'), 'Symfony Web controller must depend on the Kernel health contract.');
...
$assert(str_contains($routes, 'path: /cos/architecture/health'), 'Architecture health path must be canonical in Symfony.');
...
echo "Visualization V0.5.5 Symfony Architecture Health contract passed.\n";
