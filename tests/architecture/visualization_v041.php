...
foreach ([
    'BootstrapRuleProvidingModuleInterface', 'BootstrapPolicyProvidingModuleInterface',
    'appendBootstrapRules', 'appendBootstrapPolicies', "'bootstrap_default'",
    "'manifest.kernel_constraint'", "'runtime_contract'",
] as $marker) {
...
$assert(!str_contains($controller, 'Infrastructure\\Visualization'), 'Symfony Architecture controller must remain free of Infrastructure compile-time dependencies.');

$routes = $read('symfony/config/routes.yaml');
$assert(str_contains($routes, 'cos_web_architecture_graph:'), 'Server-side Architecture Graph endpoint route missing.');
$assert(str_contains($routes, 'path: /cos/architecture/graph'), 'Canonical graph endpoint path missing.');
...
echo "Visualization V0.4.1 Symfony hardening architecture passed.\n";
