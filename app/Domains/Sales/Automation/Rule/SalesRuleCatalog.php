        ...
        return [
            new Rule(
                ...
                [
                    ...
                    ['field' => 'deal.stage_code', 'operator' => '=', 'value' => 'NEW'],
                ],
                [
                    'type' => 'CREATE_ACTION',
            ...
            new Rule(
                $this->id($organizationId, 'sales-stage-without-next-contact-v1'),
                $organizationId,
                'Активний Deal без наступного контакту',
                DealStageChanged::TYPE,
                [
                    ['field' => 'deal.status', 'operator' => '=', 'value' => ClientCaseStatus::Active->value],
                    ['field' => 'deal.stage_code', 'operator' => 'IN', 'value' => [
                        'QUALIFIED',
                        'MEETING',
                        'PROPOSAL',
                        'NEGOTIATION',
            ...
            new Rule(
                $this->id($organizationId, 'sales-overdue-followup-escalation-v1'),
                $organizationId,
            ...
            new Rule(
                $this->id($organizationId, 'rule-call-analysis-v1'),
                $organizationId,
            ...
            new Rule(
                $this->id($organizationId, 'sales-no-activity-48h-v1'),
                $organizationId,
                'Deal без активності понад 48 годин',
                SalesEventType::NO_ACTIVITY_DETECTED,
                [['field' => 'deal.no_activity_48h', 'operator' => '=', 'value' => true]],
                [
                    'type' => 'CREATE_ACTION',
                    'action_type' => 'sales.create_followup',
                    'target_type' => 'deal',
                    'target_id' => '{{event.aggregate_id}}',
                    'parameters' => ['title' => 'Відновити контакт', 'due_in_minutes' => 60],
                    'execution_mode' => 'AUTO',
                    'risk_level' => 'MEDIUM',
                ],
                priority: 25,
            ...
            new Rule(
                $this->id($organizationId, 'sales-incoming-without-response-v1'),
                $organizationId,
                'Вхідне повідомлення потребує відповіді',
                ...
                [
                    ...
                    'target_type' => 'deal',
                    'target_id' => '{{event.aggregate_id}}',
                    'parameters' => ['title' => 'Відповісти клієнту', 'due_in_minutes' => 30],
                    'execution_mode' => 'AUTO',
                    'risk_level' => 'HIGH',
                ],
                priority: 40,
            ...
            new Rule(
                ...
                [
                    'type' => 'CREATE_ACTION',
                    'action_type' => 'sales.request_manager_review',
                    'target_type' => 'deal',
                    'target_id' => '{{event.aggregate_id}}',
                    'parameters' => ['title' => 'Перевірити Deal, що застряг', 'due_in_minutes' => 240],
                    'execution_mode' => 'AUTO',
                    'risk_level' => 'MEDIUM',
                ],
                priority: 20,
            ...
            new Rule(
                $this->id($organizationId, 'sales-meeting-without-followup-v1'),
                $organizationId,
                'Зустріч без follow-up',
                ...
                [
                    ...
                    'target_type' => 'deal',
                    'target_id' => '{{event.aggregate_id}}',
                    'parameters' => ['title' => 'Follow-up після зустрічі', 'due_in_minutes' => 1440],
                    'execution_mode' => 'AUTO',
                    'risk_level' => 'LOW',
                ],
                priority: 30,
            ...
            new Rule(
                $this->id($organizationId, 'sales-lost-reason-missing-v1'),
                $organizationId,
                'Втрачений Deal без причини',
                ...
                [
                    ...
                    'target_type' => 'deal',
                    'target_id' => '{{event.aggregate_id}}',
                    'parameters' => ['title' => 'Вказати причину втрати', 'due_in_minutes' => 240],
                    'execution_mode' => 'AUTO',
                    'risk_level' => 'MEDIUM',
                ],
                priority: 35,
            ...
            new Rule(
                $this->id($organizationId, 'sales-high-value-no-activity-v1'),
                $organizationId,
                'Великий Deal без активності',
                SalesEventType::NO_ACTIVITY_DETECTED,
                [
                ...
                [
                    ...
                    'target_type' => 'deal',
                    'target_id' => '{{event.aggregate_id}}',
                    'parameters' => ['title' => 'Терміново перевірити великий Deal', 'due_in_minutes' => 60],
                    'execution_mode' => 'AUTO',
                    'risk_level' => 'HIGH',
                ],
                priority: 50,
