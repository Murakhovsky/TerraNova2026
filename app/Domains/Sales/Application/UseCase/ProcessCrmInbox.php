...
final readonly class ProcessCrmInbox
{
    public function __construct(
        ...
        private ?RecordIncomingMessage $recordIncomingMessage = null,
    ) {
    }
        ...
        try {
            $this->transactions->transactional(function () use ($item): void {
                ...
                if (($applied['payload']['requested_message'] ?? false) === true) {
                    if($this->recordIncomingMessage===null)throw new RuntimeException('Incoming message service is unavailable for CRM ingress.');
                    $message=$this->recordIncomingMessage->execute($item->organizationId,(string)$applied['aggregate_id'],(string)($applied['payload']['channel']??'WEB'),(string)($applied['payload']['sender']??''),(string)($applied['payload']['recipient']??''),(string)$applied['payload']['body'],(string)$applied['payload']['external_id'],$item->correlationId,['provider'=>$item->provider]);
                ...
                if ($applied['aggregate_type'] === 'deal' && $requestedStage !== null && $requestedStage !== '') {
                    ...
                    $result = $this->changeDealStage->execute(new ChangeDealStageCommand($item->organizationId,(string)$applied['aggregate_id'],$stage->id,'INTEGRATION',$item->provider,$item->correlationId));
