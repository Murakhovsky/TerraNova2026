...
final readonly class CreateFollowupTaskHandler implements ActionHandlerInterface
{
    ...
    public function execute(Action $action): ExecutionResult
    {
        ...
        if ($dueAt === null && isset($action->parameters['due_in_minutes'])) {
            $minutes = max(1, (int) $action->parameters['due_in_minutes']);
            $dueAt = (new DateTimeImmutable())->modify(sprintf('+%d minutes', $minutes));
        }
        ...
        if (!$result->successful) {
            return ExecutionResult::failure($result->error ?? 'CRM task creation failed.', $result->data);
        }
