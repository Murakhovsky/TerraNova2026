...
namespace Domains\Content\Infrastructure\Persistence\MySql;
...
final class MysqlContentRepository implements ContentRepositoryInterface
{
    ...
    public function __construct(
        ...
        private ContentIntegrationOutboxInterface $integrationOutbox,
    )
    {
    }
    ...
    {
        return $this->integrationOutbox->stats('n8n');
    }
    ...
    public function save(array $input, ?array $user = null, string $source = 'manual', bool $emitEvent = true): array
    {
        ...
        try {
            ...
            if ($emitEvent) {
                $this->queueOutbound('content.changed', $id, [
                    'id' => $id,
    ...
    private function queueOutbound(string $eventType, int $id, array $payload): void
    {
        $this->integrationOutbox->enqueue('n8n', $eventType, 'content', $id, $payload);
    }
