...
namespace Domains\Spatial\Infrastructure\Persistence\MySql;
...
final class MysqlSpatialSceneRepository implements SpatialSceneRepositoryInterface
{
    ...
    public function __construct(
        ...
        private SpatialAssetStorageInterface $assets,
        private PropertyTourPublisherInterface $propertyTours,
    )
    {
    }
    ...
    public function publish(int $sceneId): array
    {
        ...
        if (!empty($scene['property_id'])) {
            $this->propertyTours->publishTour(
                (int) $scene['property_id'], '/spatial/scene/' . $scene['slug'],
            );
        }
