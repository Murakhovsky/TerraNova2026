...
namespace Domains\Property\Infrastructure\Presentation;
...
class PropertyPresentationService implements PropertyPresentationInterface
{
    ...
    public function __construct(
        ...
        private ?PresentationSalesInterface $sales = null,
    ) {
    }
    ...
    public function recordDownload(array $document, ?array $user, string $sourcePage): void
    {
        try {
            ...
            $this->analytics?->recordPresentation(
                'presentation_download', $propertyId, null,
                !empty($user['id']) ? (int) $user['id'] : null, $sourcePage,
                ['variant' => $document['variant'], 'kind' => $document['kind'], 'group_id' => $document['group_id']],
            );
    ...
    public function registerShare(array $input, ?array $user = null): array
    {
        ...
        $case = $this->sales?->activeClientCase($caseId);
        ...
        try {
            ...
            $propertyId = !empty($document['property_id']) ? (int) $document['property_id'] : null;
            $this->sales?->recordShare(
                $caseId, (int) $case['person_id'], !empty($user['id']) ? (int) $user['id'] : null,
                'Презентацію підготовлено для надсилання',
                $channelLabels[$channel] . ': ' . $document['title'] . "\n" . $pdfUrl,
                $propertyId, 'Презентація: ' . $channelLabels[$channel],
            );
            ...
            $this->analytics?->recordPresentation(
                'presentation_share', !empty($document['property_id']) ? (int) $document['property_id'] : null,
                $caseId, !empty($user['id']) ? (int) $user['id'] : null, $pageUrl,
                ['client_case_id' => $caseId, 'channel' => $channel, 'variant' => $variant,
                    'kind' => $document['kind'], 'group_id' => $document['group_id']],
            );

            $pdo->commit();
            if (!empty($document['property_id'])) {
        $this->notifications?->notifyPresentationShared(
