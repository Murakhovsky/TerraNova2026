...
final readonly class GrowthLinkedInHandler implements IdempotentExternalActionHandlerInterface
{
    public const TYPE='growth.send_linkedin';
    ...
    public function execute(Action $action):ExecutionResult
    {
        ...
        if(strtolower(trim((string)($action->parameters['channel']??'')))!=='linkedin'){
            return ExecutionResult::failure('Growth LinkedIn action requires linkedin channel.');
        }
        $body=trim((string)($action->parameters['body']??''));
        if($body===''||mb_strlen($body)>10000){
            return ExecutionResult::failure('Growth LinkedIn message body must be 1..10000 characters.');
        }
        ...
        if(strtolower(trim((string)($contact['identity_type']??'')))!=='linkedin'){
            return ExecutionResult::failure('Growth contact has no LinkedIn identity.');
        }
        $profile=trim((string)($contact['identity_value']??''));

        try{
            $delivery=$this->outbound->queueLinkedIn(
            ...
            return ExecutionResult::failure(
                'Growth LinkedIn integration unavailable: '.$error->getMessage(),
        ...
        if($delivery->status==='failed'){
            return ExecutionResult::failure(
                'Growth LinkedIn delivery is currently failed.',
