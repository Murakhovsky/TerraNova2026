...
final readonly class MysqlGrowthMutationReceipt implements GrowthMutationReceiptInterface
{
    ...
    public function claim(string $organizationId,string $operation,string $idempotencyKey,string $fingerprint): bool
    {
        $statement=$this->connection->prepare(
            'INSERT IGNORE INTO tn_growth_operation_receipts
        ...
        $lookup=$this->connection->prepare(
            'SELECT payload_fingerprint FROM tn_growth_operation_receipts
        ...
        if($existing===false)throw new InvalidArgumentException('Growth idempotency claim could not be resolved.');
        if(!hash_equals((string)$existing,$fingerprint)){
            throw new InvalidArgumentException('Idempotency key was reused with a different Growth payload.');
        }
        return false;
    }
}
