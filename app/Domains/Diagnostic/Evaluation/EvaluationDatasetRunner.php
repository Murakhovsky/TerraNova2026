...
/**
 * Generic deterministic evaluation-dataset runner.
 *
 * Bounded contexts may wrap this service for domain-specific benchmark datasets,
 * but the Diagnostic core never imports those contexts.
 */
final readonly class EvaluationDatasetRunner
{
    ...
    public function run(CompiledDiagnosticPack $pack, array $dataset): array
    {
        ...
        foreach ($dataset['cases'] ?? [] as $case) {
            ...
            $facts = [];
            foreach ($case['input_facts'] as $id => $value) {
                $facts[$id] = new ObservedValue($value, [$signal]);
            }
            $metrics = [];
            foreach ($case['input_metrics'] as $id => $value) {
                $metrics[$id] = new ObservedValue($value, [$signal]);
            }
            ...
            foreach ($pack->recommendationsById as $recommendation) {
                if (array_intersect($recommendation->triggerRules, $actual)) {
                    $actualRecommendations[] = $recommendation->id;
                }
            }
        ...
        $universe = [];
        foreach ($dataset['cases'] ?? [] as $case) {
            foreach ($pack->rulesById as $id => $rule) {
                $universe[] = $case['id'] . ':' . $id;
            }
        }
