...
final readonly class MysqlSalesAgentContextBuilder implements AgentContextBuilderInterface
{
    ...
    public function build(AgentInvocation $invocation): array
    {
        ...
        $statement->execute(['id' => $invocation->subjectId, 'organization_id' => $invocation->organizationId]);
        ...
        $activities = $this->connection->prepare(
            ...
            . 'FROM tn_client_case_activities WHERE client_case_id = :id AND organization_id = :organization_id '
            . 'ORDER BY created_at DESC LIMIT 30'
        );
        $activities->execute(['id' => $invocation->subjectId, 'organization_id' => $invocation->organizationId]);
