...
class Lists extends PresentationModel
{
    ...
    public static function getListByObjectId($object_id, $my_id){
        ...
        JOIN ". ListItems::class . " AS fi ON fi.list_id = fl.id
...
//        return $queryBuilder->getQuery()->execute();
    ...
    public function getTGCard(int $user_id, array $params)
    {
        $inline_keyboard = array(
            array(
                InlineKeyboardFactory::button(TG_OBJECTS, 'fid_' . $this->id, 'switch_inline_query_current_chat'),
                InlineKeyboardFactory::button(TG_OBJECTS . ' ⤴', 'fid_' . $this->id, 'switch_inline_query')
            )
        );

        if($user_id == $this->user_id) {
            $inline_keyboard[] = InlineKeyboardFactory::adminLine('favourite', $this->id);
        }
        $inline_keyboard[] = InlineKeyboardFactory::controlLine('favourite,' . $this->id);
