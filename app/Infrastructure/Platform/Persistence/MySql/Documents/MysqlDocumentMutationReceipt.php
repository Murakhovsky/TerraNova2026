...
final readonly class MysqlDocumentMutationReceipt implements DocumentMutationReceiptInterface
{
    ...
    public function claim(
        ...
        $statement=$this->connection->prepare(
            'INSERT IGNORE INTO cos_document_operation_receipts
        ...
        $lookup=$this->connection->prepare(
            'SELECT payload_fingerprint FROM cos_document_operation_receipts
        ...
        if($existing===false)throw new InvalidArgumentException('Document idempotency claim could not be resolved.');
        if(!hash_equals((string)$existing,$fingerprint)){
            throw new InvalidArgumentException('Idempotency key was reused with a different Documents payload.');
        }
        return false;
    }
}
