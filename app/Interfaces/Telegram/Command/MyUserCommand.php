...
abstract class MyUserCommand extends UserCommand
{
//    private $name;
...
//                $this->notes['source'] = str_replace('search', '', $this->name);
                ...
                $search_result = Realty::searchRealty($this->notes, $page, 5);
...
//        );
    ...
    protected function createPagination($page = 1){
        $lastPage = 50;
        $selectedPage = max(1, min($lastPage, (int) $page));
        $messageId = $selectedPage === 1 ? (int) $this->message_id + 1 : $this->message_id;
        $startPage = max(1, min($selectedPage - 3, $lastPage - 6));
        $endPage = min($lastPage, $startPage + 6);
        $keyboard = [];

        for ($targetPage = $startPage; $targetPage <= $endPage; $targetPage++) {
            $label = $targetPage === $selectedPage ? "· {$targetPage} ·" : (string) $targetPage;
            if ($targetPage === 1 && $selectedPage > 4) {
                $label = "« {$targetPage}";
            } elseif ($targetPage === $lastPage && $selectedPage < $lastPage - 3) {
                $label = "{$targetPage} »";
            } elseif ($targetPage === $selectedPage - 1) {
                $label = "‹ {$targetPage}";
            } elseif ($targetPage === $selectedPage + 1) {
                $label = "{$targetPage} ›";
            }

            $keyboard[] = Buttons::getButton(
                $label,
                "{$this->name};command;search;{$selectedPage};{$targetPage};{$messageId}"
            );
        }
...
//        }
        ...
        }catch (DatabaseOperationFailed $e){
