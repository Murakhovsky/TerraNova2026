...
readonly class ClientCaseService
{
    public function __construct(
        ...
        private CreateClientCase $createCase,
        private UpdateClientCase $updateCase,
        private QuickUpdateClientCase $quickUpdateCase,
        private AddClientCaseActivity $addCaseActivity,
        private AttachInboundRequest $attachRequest,
        private UpdateInboundClientCaseRequest $updateInbound,
        private CreateClientCaseFromInboundRequest $createFromInbound,
        private AddClientCasePropertyMatch $addProperty,
        private UpdateClientCasePropertyMatch $updateProperty,
        private RegisterInboundClientCaseRequest $registerInbound,
        private ResolveInboundProperty $resolveInboundProperty,
        private EnsureInboundClientCase $ensureInboundCase,
    ) {
    }

    public function filters(array $query): array
    {
        return $this->readModel->filters($query);
    }

    public function cases(array $filters): array
    {
        return $this->readModel->cases($filters);
    }
    ...
    {
        return $this->readModel->case($id);
    }

    public function inboundRequests(int $caseId): array
    {
        return $this->readModel->inboundRequests($caseId);
    }

    public function activities(int $caseId): array
    {
        return $this->readModel->activities($caseId);
    }

    public function propertyMatches(int $caseId): array
    {
        return $this->readModel->propertyMatches($caseId);
    }

    public function requestMatches(int $caseId): array
    {
        return $this->readModel->requestMatches($caseId);
    }

    public function unlinkedInboundRequests(): array
    {
        return $this->readModel->unlinkedInboundRequests();
    }

    public function inboundFilters(array $query): array
    {
        return $this->readModel->inboundFilters($query);
    }

    public function inboundInbox(array $filters): array
    {
        return $this->readModel->inboundInbox($filters);
    }

    public function inboundInboxStats(): array
    {
        return $this->readModel->inboundInboxStats();
    }

    public function leadActivities(int $leadId): array
    {
        return $this->readModel->leadActivities($leadId);
    }

    public function openCaseOptions(): array
    {
        return $this->readModel->openCaseOptions();
    }

    public function managerOptions(): array
    {
        return $this->readModel->managerOptions();
    }

    public function create(array $input, ?array $user = null): array
    {
        try {
            $result = $this->createCase->execute($input, $user);
            if (!$result->ok && $result->code === 'contact_required') {
            ...
            $this->logError('client-case-create', $error);
    ...
    {
        try {
            $result = $this->updateCase->execute($caseId, $input, $user);
            return ['ok' => $result->ok, 'message' => $result->ok ? 'Кейс оновлено.' : 'Кейс не знайдено.'];
        } catch (Throwable $error) {
            $this->logError('client-case-update', $error);
    ...
    {
        try {
            $result = $this->quickUpdateCase->execute($caseId, $input, $user);
            return ['ok' => $result->ok, 'message' => $result->ok ? 'Кейс оновлено.' : 'Кейс не знайдено.'];
        } catch (Throwable $error) {
            $this->logError('client-case-quick-update', $error);
            return ['ok' => false, 'message' => 'Кейс не вдалося оновити.'];
        }
    }
    ...
    {
        try {
            $result = $this->addCaseActivity->execute($caseId, $input, $user);
            return ['ok' => $result->ok, 'message' => $result->ok ? 'Дію додано.' : 'Кейс не знайдено.'];
        } catch (Throwable $error) {
            $this->logError('client-case-activity', $error);
    ...
    {
        try {
            $result = $this->attachRequest->execute($caseId, $requestId, $user);
            $message = $result->ok ? 'Заявку привʼязано до кейсу.'
                : ($result->code === 'case_not_found' ? 'Кейс не знайдено.' : 'Заявку не знайдено.');
            return ['ok' => $result->ok, 'message' => $message];
        } catch (Throwable $error) {
            $this->logError('client-case-attach-inbound-request', $error);
    ...
    {
        try {
            $result = $this->updateInbound->execute($requestId, $input, $user);
            return [
                'ok' => $result->ok,
                'message' => $result->ok ? 'Заявку оновлено.' : 'Заявку не знайдено.',
                'case_id' => $result->data['case_id'] ?? null,
            ];
        } catch (Throwable $error) {
    ...
    {
        try {
            $result = $this->createFromInbound->execute($requestId, $input, $user);
            $message = match ($result->code) {
                'already_attached' => 'Заявка вже привʼязана до кейса.',
                'request_not_found' => 'Заявку не знайдено.',
                default => 'Кейс створено із заявки.',
            };
    ...
    {
        try {
            $result = $this->addProperty->execute($caseId, $propertyId, $input, $user);
            $message = match ($result->code) {
                'case_not_found' => 'Кейс не знайдено.',
                'property_not_found' => 'Обʼєкт не знайдено.',
                default => 'Обʼєкт додано до кейсу.',
            };
            return ['ok' => $result->ok, 'message' => $message, 'case_id' => $result->data['case_id'] ?? $caseId];
        } catch (Throwable $error) {
            $this->logError('client-case-property-match-add', $error);
    ...
    {
        try {
            $result = $this->updateProperty->execute($matchId, $input, $user);
            return [
                'ok' => $result->ok,
                'message' => $result->ok ? 'Підбір оновлено.' : 'Підбір не знайдено.',
                'case_id' => $result->data['case_id'] ?? null,
            ];
        } catch (Throwable $error) {
    ...
    {
        $this->registerInbound->execute($caseId, $requestId);
    }

    public function addInboundPropertyMatch(int $caseId, int $propertyId): void
    {
        if ($this->resolveInboundProperty->execute($propertyId) === null) return;
        $this->addProperty->execute($caseId, $propertyId, [
            'match_status' => 'interested', 'note' => 'Обʼєкт із вхідної заявки',
        ]);
    }

    public function inboundPropertyId(mixed $value): ?int
    {
        return $this->resolveInboundProperty->execute($value);
    }

    public function ensurePersonAndCaseFromInbound(array $input, ?array $user = null): array
    {
        try {
            $result = $this->ensureInboundCase->execute($input, $user);
            return [
    ...
    private function logError(string $label, Throwable $error): void
    {
        ...
        @file_put_contents(
            $directory . '/frontend.log',
            sprintf("[%s] %s: %s%s", date('Y-m-d H:i:s'), $label, $error->getMessage(), PHP_EOL),
            FILE_APPEND,
        );
    }
}
