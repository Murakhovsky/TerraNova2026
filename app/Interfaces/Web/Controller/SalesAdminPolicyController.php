...
final class SalesAdminPolicyController extends WebController
{
    public function actionsAction(): void
    {
        ...
        $this->view->title = 'Actions & Policies Administration';
        $this->view->metaTitle = 'Sales Policies | Terra Nova COS';
        ...
        $this->view->pick('sales_admin/actions');

        try {
            $this->view->actions = $this->service()->actions($this->organization()->id());
            $this->view->policyCatalog = $this->service()->catalog($this->organization()->id());
        } catch (Throwable $error) {
            $this->renderFrontendException($error, 'sales_admin.policies');
        }
    }

    private function service(): SalesPolicyAdministrationInterface
    {
        return $this->di->getShared('salesPolicyAdministration');
    }
}
