...
- New public Property reads use `/api/v1/public/properties*`. `/api/v1/properties*` is the authenticated manager/canonical business API. Public favourites use the canonical session-backed `/api/v1/public/properties/favourites` endpoint; the former Phalcon compatibility path is retired.
