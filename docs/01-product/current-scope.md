...
| Growth | `0.31.0` | Opportunity Intelligence + automatic Signal monitoring + tenant-safe Polling Operations visibility; schema `0.28.0`; disabled by default |
...
Growth `0.31.0` розвиває окремий bounded context для **FIND VALUE**. Canonical runtime визначає `Signal`, `OpportunityCandidate`, `OpportunityRationale`, explainable Fit/Need/Timing/Access/Value scoring, lifecycle qualification та `OpportunityHandoff`.
